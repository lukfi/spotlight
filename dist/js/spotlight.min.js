...
function U(a,b){var c=document.createElement("div");c.className="spl-"+a;h(c,"click",b);Qa.appendChild(c);return ia[a]=c}function cb(a){var b=a.target.closest(".spotlight");if(b){ca(a,!0);a=b.closest(".spotlight-group");D=(a||document).getElementsByClassName("spotlight");for(var c=0;c<D.length;c++)if(D[c]===b){xa=a&&a.dataset;rb(c+1);break}}}
...
function Bb(){return document.fullscreen||document.fullscreenElement||document.webkitFullscreenElement||document.mozFullScreenElement}function Cb(){sb();M&&zb();if(T){var a=Bb();d(Xa,"on",a);a||l(Xa,0<screen.availHeight-window.innerHeight)}}function sb(){u=q.clientWidth;v=q.clientHeight}function zb(){na=M.clientWidth;oa=M.clientHeight}function Z(a,b){g(L,"transform",a||b?"translate("+a+"px, "+b+"px)":"")}
...
function ob(a){ca(a,!0);ta=!0;ua=!1;var b=a.touches;if(b){if(2===b.length){pa=Math.hypot(b[0].pageX-b[1].pageX,b[0].pageY-b[1].pageY);return}b=b[0];var c=Date.now(),e=b.pageX,f=b.pageY;if(300>c-qa&&30>Math.abs(e-ra)&&30>Math.abs(f-sa)){1<w?V():hb();qa=0;ta=!1;return}qa=c;ra=e;sa=f}va=na*w<=u;la=a.pageX||b.pageX;ma=a.pageY||b.pageY;n(L);g(L,"transition","");g(M,"transition","")}
function qb(a){ca(a);if(ta){pa=0;if(ua){if(va&&ua){var b=(a=r<-(u/7)&&(B<C||F))||r>u/7&&(1<B||F);if(a||b)Db(B-1,!0,r/u*100),a&&nb()||b&&mb();r=0;Z()}n(L,!0)}else Ib();ta=!1}}
function pb(a){ca(a);if(ta){var b=a.touches;if(b){if(2===b.length&&!1!==E["zoom-in"]){var c=Math.hypot(b[0].pageX-b[1].pageX,b[0].pageY-b[1].pageY);if(0<pa&&(a=c/pa,.01<Math.abs(a-1))){a*=w;a=Math.max(1,Math.min(5,a));if(1===a)t=r=0;else if(1<=a&&5>=a){y&&V();var e=L.getBoundingClientRect(),f=(b[0].pageX+b[1].pageX)/2-e.left;b=(b[0].pageY+b[1].pageY)/2-e.top;e=(b-v/2-t)/w;r=f-u/2-(f-u/2-r)/w*a;t=b-v/2-e*a;b=Math.max(0,(na*a-u)/2);f=Math.max(0,(oa*a-v)/2);r=Math.max(-b,Math.min(b,r));t=Math.max(-f,
Math.min(f,t))}w=a;g(L,"transition","transform 0.3s ease-out");g(M,"transition","transform 0.3s ease-out");Z(r,t);g(M,"transform","translate(-50%, -50%) scale("+w+")");pa=c}return}b=b[0]}e=Math.max(1,.15*w);c=(na*w-u)/2;f=(la-(b?b.pageX:a.pageX))*e;e*=ma-(b?b.pageY:a.pageY);la=b?b.pageX:a.pageX;ma=b?b.pageY:a.pageY;va?r-=f:(b=r-f,a=t-e,r=Math.abs(b)>c?0<b?c+.5*(b-c):-c+.5*(b+c):b,oa*w>v&&(c=(oa*w-v)/2,t=Math.abs(a)>c?0<a?c+.5*(a-c):-c+.5*(a+c):a));ua=!0;g(L,"transition","none");Z(r,t)}else X()}
function fb(a){var b=Bb();if("boolean"!==typeof a||a!==!!b)if(b)document[bb]();else q[T]()}function kb(a){"string"!==typeof a&&(a=z?"":Ea||"white");z!==a&&(z&&d(q,z),a&&d(q,a,!0),z=a)}function V(a){"boolean"===typeof a&&(y=!a);y=1===w&&!y;d(M,"autofit",y);g(M,"transform","");w=1;t=r=0;zb();n(L);Z()}
function hb(){var a=w/.7;50>=a&&(y&&V(),g(L,"transition","transform 0.3s cubic-bezier(0.4, 0.0, 0.2, 1)"),g(M,"transition","transform 0.3s cubic-bezier(0.4, 0.0, 0.2, 1)"),w=a,Z(r,t),g(M,"transform","translate(-50%, -50%) scale("+w+")"))}function ib(){var a=.85*w;1<=a&&(y&&V(),1.5>w&&(a=1,t=r=0),g(L,"transition","transform 0.4s cubic-bezier(0.4, 0.0, 0.2, 1)"),g(M,"transition","transform 0.4s cubic-bezier(0.4, 0.0, 0.2, 1)"),w=a,Z(r,t),g(M,"transform","translate(-50%, -50%) scale("+w+")"))}
function gb(){Ra=!Ra;m(N,Ra)}function lb(){var a=K,b=document.createElement("a"),c=M.src;b.href=c;b.download=c.substring(c.lastIndexOf("/")+1);a.appendChild(b);b.click();a.removeChild(b)}function eb(a){setTimeout(function(){K.removeChild(q);L=M=P=E=xa=D=za=Aa=Ba=Ha=null},200);d(K,"hide-scrollbars");d(q,"show");fb(!1);ub();history.go(!0===a?-1:-2);Q&&(Oa.src="");R&&W();M&&Ab(M);S&&(S=clearTimeout(S));z&&kb();H&&d(q,H);Ba&&Ba()}
function Ab(a){if(a.g)a.g.appendChild(a),a.g=null;else{var b=a.parentNode;b&&b.removeChild(a);a.src=a.onerror=""}}function mb(a){a&&X();if(1<C){if(1<B)return Jb(B-1);if(F)return Db(C,!0),Jb(C)}}function nb(a){a&&X();if(1<C){if(B<C)return Jb(B+1);if(F)return Db(-1,!0),Jb(1);R&&W()}}function Jb(a){if(a!==B){R?(clearTimeout(R),xb()):X();var b=a>B;B=a;tb(b);return!0}}
